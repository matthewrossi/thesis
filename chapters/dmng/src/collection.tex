...
subsystem allows changing at runtime the behavior of the kernel
without changing its implementation nor adding new modules. Briefly,
it permits so by loading compact programs within the kernel,
...
provide abstractions such as arrays and hash maps. It is important to
...
subsystem automatically performs the two stages of Program
...
set of eBPF programs comprises: 1)~dedicated programs to trace the
...
programs ensures that monitoring extends to tasks spawned through the
...
application, nor it is necessary to restart it to set up the process.
...
\begin{table}[t]
  ...
  \begin{tabular}{ l }
    ...
    \bottomrule
  \end{tabular}
  \caption{List of fil system traced hook points.}
\end{table}
