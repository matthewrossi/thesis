...
\begin{table*}[t]
	...
	\textit{allowStmt} & $\to$& $($\texttt{allow} $\langle$\textit{sourceTypeId} $|$ \textit{sourceTypeAttrId}$\rangle$  $\langle$\textit{targetTypeId} $|$ \textit{targetTypeAttrId} $|$ \texttt{self}$\rangle$  \textit{classPermissionId}$+$$)$ \\

      \end{tabularx}
        \caption{Application policy module CIL syntax.\protect\footnotemark}
  \label{tab:seapp_syntax}
\end{table*}

\footnotetext{The use of different fonts distinguishes terminal symbols (in typewriter) from the non-terminal symbols (in italics).}
...
multistep) expansion of its definition.  The semantics is that each
...
local type identifier.  Also, the use of a namespace specific to the
policy module permits the differentiation between local types or
...
statements.  This approach retains control on the rules
