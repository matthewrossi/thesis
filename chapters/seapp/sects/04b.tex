...
file systems that support extended file attributes (e.g., \data), while
the latter with the ones that do not (e.g., \proc).  To apply
\filecontexts updates, two approaches are available: either rebuild
the file system image, or run \restorecon operation on the file or
...
changes, a reboot of the device or a sequence of file system {\em
  unmount} and {\em mount} operations has to be performed.
...
\begin{lstlisting}[language=policyfile, caption=\seappcontexts
  ...
  breakatwhitespace=true, postbreak=\mbox{{$\hookrightarrow$}\space},
  breakindent=1ex, captionpos=b]
...
\texttt{file\textunderscore type} and \texttt{security\textunderscore
  context}: \texttt{pathname\textunderscore regexp}
...
describes the class of file system resource (i.e., directory, file,
...
\begin{lstlisting}[language=policyfile, caption=\filecontexts
  example, label=seapp_lstfile, numbersep=2pt, resetmargins=false, captionpos=b]
...
\texttt{untrusted\textunderscore app\textunderscore
  all.te} platform policy file~\cite{seapp_untrustedappte} permits to
a process labeled with \untrustedapp to access \texttt{audioserver}, \texttt{camera},
...
\begin{lstlisting}[language=policyfile, caption=Granting \texttt{cameraserver}
  access to secret domain, label=seapp_lstdomains, numbersep=2pt, resetmargins=false, captionpos=b]
(block package_name
  (type secret)
