...
performs the built-in functions listed there, among them mounting the
...
build and reload of the policy, we implemented a new built-in function,
...
inspects the APK to identify whether the current installation
...
class responsible for loading the appropriate \macpermissions file and assigning \seinfo values
...
role is to spawn and correctly set up the new application process.  To
...
isolation among source and target domains managing the multithreaded
...
of the default security context inheritance behavior for the {\em
  ext4} file system, (ii) execution of the \sel \restorecon operation
