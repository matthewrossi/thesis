...
The fs element is used to configure file-system-related permissions. Fs
stores three optional arrays: {\em read}, {\em write} and {\em
  exec}. File system paths are used as array values. As an example, the
...
used to replace any of the {\em fs}, {\em read}, {\em write} and {\em
  exec} arrays to grant access to the whole file system.
...
associated with their application. Similarly to the file system case, the
...
file system resources, so, while the granularity of the six flags
...
pipe (pinned to the file system) by using its fully qualified path.
...
file system, the policy permits granting access to the network without
...
%
 To handle IP address migrations that may occur at runtime,
  ...
  extended monitoring DNS traffic on port 53, hence providing support
