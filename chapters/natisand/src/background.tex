...
file system to read configuration files. To address these requirements,
...
pattern are: interaction with the file system (e.g., fs, Deno.FsFile),
...
executing code outside the isolated context managed by the engine,
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{chapters/natisand/fig/js_runtime}
...
associate a set of permissible actions with a file system path (e.g.,
...
currently, the protection offered is only limited to the file system.
...
program, data structures named {\em maps} are used. Maps also enable
sharing data among eBPF programs and user space
applications.
...
Modern eBPF development is facilitated by the presence of {\em
  frontends}. These frameworks permit writing eBPF programs in a C
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\columnwidth]{chapters/natisand/fig/bpf_bg}
...
available to a user space application. The rationale is that the
...
extended in 2012, and it now permits intercepting system calls and
