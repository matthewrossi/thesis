...
prevents a process from modifying the host file system by layering the
sandbox file system on top of it. The solution is implemented by
...
either require a privileged daemon or require loading kernel modules
...
security mechanisms used are: introduction of new user identifiers,
...
compiled languages, so while some concepts are portable to JS
...
proposal.
{\em Cage4deno}~\cite{cage4deno} protects file system resources from subprocesses
...
to subprocess, file system, and network resources for the JS
code. Similarly, \natisand takes care of protecting file system, IPC, and
...
native code when accessing file system resources. Another research work
