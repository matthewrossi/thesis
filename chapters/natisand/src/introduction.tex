...
cloud applications is quite established in the industry. In this
...
% Restricting access to file system resources significantly limits the
...
% to secure file system, IPC and network resources in cloud applications
...
file system. Runtimes also provide support for the execution of native
...
in native languages such as C or C++.  By reusing existing utilities,
developers can take advantage of popular high-performance libraries,
which, in addition to improving performance, minimizes the cost of
...
adversary to corrupt the file system, perform privilege escalation,
...
developer to control on a native-component basis, access to file system, Inter-Process
...
of our approach is that it permits sandboxing native code while
preserving backward compatibility; namely, it does not require
changing existing modules (including third-party dependencies) to
