...
sandboxing to complete isolation, a valuable solution should permit
starting by sandboxing the components associated with the highest risk,
...
\natisand separates system resources into three categories: file system,
...
restrict access to each resource category.  File-system-related
...
not require leveraging root permissions while the application is
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{chapters/natisand/fig/overview_sandbox}
...
that any request to access file system, IPC, and network resources will
...
\natisand restricts access to the file system using Landlock. The sandbox
...
high resource utilization. Parallel execution typically requires
handling synchronization and communication between a parent and a group
of child tasks. In this setting, best practice suggests providing the
...
\paragraph{Network}
%
\natisand permits controlling how each isolated context connects to network
...
\end{table}
  ...
  of our solution, therefore they could be used in conjunction with
