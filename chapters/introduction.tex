...
specifics of emerging technologies that enable finer-grained
...
for improvement. Here, not only our approach permits the introduction of
fine-grained policies to restrict file system access, it also replaces
error-prone user space implementations of the security checks (and the
...
file system, Inter-Process Communication (IPC), and network resources
...
\textbf{Chapter~\ref{chap:intro}} illustrates the document
structure and the publications that set the basis for this book.

\textbf{Chapter~\ref{chap:methodology}} showcases the
...
\textbf{Chapter~\ref{chap:seapp}} describes \seapp~\cite{seapp}, a
...
\textbf{Chapter~\ref{chap:dmng}} highlights the limitations of
...
\noindent The chapter is organized as follows.
\begin{compactitem}
    \item Section~\ref{dmng:sect:introduction} presents the challenges
...
\textbf{Chapter~\ref{chap:wasm}} explores the use of WebAssembly
...
\textbf{Chapter~\ref{chap:natisand}} considers the use of
...
{\em Seccomp} to control the file system, Inter-Process Communication
...
\begin{compactitem}
    ...
    \item Section~\ref{sectbackground} overviews the structure of
     modern JS runtimes and provides background on the components
...
\textbf{Chapter~\ref{chap:conclusions}} draws the conclusions and
discusses future work.

\section{Publications}

This section presents the list of publications that set the basis for this book.

\subsubsection*{Thesis}

\begin{itemize}
    \nocite{thesis}
    \item Matthew Rossi, ``\textbf{Fine-grained Access Control
        Technologies to Protect Resources in Mobile and Cloud
        Applications}''. Università degli Studi di Bergamo. 2024.
\end{itemize}
...
\begin{itemize}
    ...
    \item Sabrina De Capitani di Vimercati, Dario Facchinetti, Sara
        ...
        Indexing for Encrypted Data}''. IEEE Transactions on Cloud
        Computing (TCC), Volume 12, Issue 3. IEEE 2024.
\end{itemize}
...
\begin{itemize}
    \nocite{syscalls}
    \item Matthew Rossi, Michele Beretta, Dario Facchinetti, and
        Stefano Paraboschi. ``\textbf{Transparent
        Temporally-Specialized System Call Filters}''. {\em
        Proceedings of the 2025 ACM Asia Conference on Computer and
        Communications Security (ASIACCS)}. ACM, 2025.

    \nocite{secure-scheduling}
    \item Matthew Rossi, Michele Beretta, Dario Facchinetti, and
        Stefano Paraboschi. ``\textbf{Policy-driven security-aware
        scheduling in Kubernetes}''. {\em Proceedings of the 2025 ACM
